...
frontDir = "D:/UserProfiles/MeepoChen/Desktop/1/"
...
class GetPictures(object):
    ...
    def pictures_sum(self):
        str1 = input(unicode("第几套开始，重复会跳过:", "utf8"))
        try:
            self.first_num = int(float(str1))
        except ValueError:
            print(unicode('输入的不为数字', "utf8"))

        str1 = input(unicode("收集多少套:", "utf8"))
        try:
    ...
    def get_img_sum_num(self, img_url):
        ...
        # 获取标签里面的值
        img_sum_number = int(soup.find_all('a', href=re.compile('/mm'))[8].get_text().strip())
        print(unicode("总共找到%d文件" % img_sum_number, "utf8"))
    ...
    def down_pictures(self, img_urls):
        ...
        img_name = frontDir + img_name
        if os.path.exists(img_name):    # 查重 如果这个文件夹存在则跳过 防止重复下载
            time.sleep(1)
            print(unicode(img_name+'存在', "utf8"))
            return
        os.mkdir(img_name)
        for img_url in img_urls:
            ...
            with open(img_name + u'/' + img_url.split('/')[-1], 'wb') as f:
                ...
                f.close()
                print(unicode("已保存" + img_name + '/' + img_url.split('/')[-1], "utf8"))
